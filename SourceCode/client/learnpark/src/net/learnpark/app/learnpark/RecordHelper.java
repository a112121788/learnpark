...
public class RecordHelper implements OnCompletionListener, OnErrorListener {
	...
	public void startRecording(int outputfileformat, String extension,
		...
		mSampleFile = new File(new DateFormat().format("yyyyMMdd_hhmm",
				Calendar.getInstance(Locale.CHINA)) + "");
		Log.i("ss", mSampleFile + "1111111111111111111111111");
		String aString = new DateFormat().format("yyyyMMdd_hhmm",
				Calendar.getInstance(Locale.CHINA))
				+ "";
		Log.i("recode", aString);
		File sd = Environment.getExternalStorageDirectory();
		String path = sd.getPath() + "/learnpark";
		...
		// the device.
		sampleDir = new File("/sdcard/learnpark/");
		...
		try {
			...
			boolean isInCall = ((audioMngr.getMode() == AudioManager.MODE_IN_CALL) || (audioMngr
					.getMode() == AudioManager.MODE_INVALID));
			if (isInCall) {
